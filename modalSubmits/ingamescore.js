    ...
    async onSubmit(interaction, channel, channelPermissions, bot) {
        ...
        try {
            ...
            if (updateSheet.status === 200) {
                await interaction.editReply({ content: `Yatta!! ${interaction.member.nickname || interaction.member.user.username} sent an in-game score of ${ingameScore} to ${clanBattleNum}`, ephemeral: true})

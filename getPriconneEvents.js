...
const getEvents = async () => {
    ...
    const events = await page.evaluate(() => {
        for (let i = 2; i < 6; i++) {
            if (document.querySelectorAll(`.contents ul:nth-last-child(${i})`).length === 1) {
                return Array.from(document.querySelectorAll(`.contents ul:nth-last-child(${i}) > li`)).map((element) => element.textContent)
            }
        }
    })
    await browser.close()
    ...
    const headline = events.map((event) => event.split('\n')[0])
                        .filter((event) => event.match(findDateTime))
    const eventInfo = headline.map((header) => {
        return { "eventName": header.match(findStringBeforeDate)[1].trim(),
